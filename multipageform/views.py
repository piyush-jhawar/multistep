...
class MultipageFormView(FormView):
    # must be overridden in subclass (as "form_class")
    mp_form_class = ""
    ...
    def as_view(cls, **initkwargs):
        ...
        assert any([cf for cf in child_forms if cf ==
                   mp_form_class.starting_form]), NO_MATCHING_FORM
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.instance:
            ...
            if request.method == "POST":
                self.instance = self.mp_form_class.model(
        ...
        # The user has passed the starting form and needs the next class.
            ...
            self.form_class = self.child_forms[self.request.session.get(
                "next_form_class")]

        # If this form class has its own template, override the default.
        if getattr(self.form_class, "template_name", None):
            print("THE REQUETS", self.form_class)
            print("template name is ", self.form_class.template_name)
    ...
    def check_future_history(self, next_form_class, instance):
        ...
        while len(self.history) > lookahead_pointer:
            if next_form_class.__name__ == self.history[lookahead_pointer]:
                ...
                next_form_instance = next_form_class(
    ...
    def get_instance_from_key(self, session_key):
        ...
        try:
            instance = self.mp_form_class.model.objects.get(
                session_key=session_key)
        except self.mp_form_class.model.DoesNotExist:
            return None
        # else
        now = datetime.datetime.utcnow().replace(tzinfo=pytz.UTC)
        instance.is_expired = instance.modified < now - \
